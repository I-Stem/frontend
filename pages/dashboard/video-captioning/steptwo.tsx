...
const Steptwo: NextPage<IStemServices.IProps, IStemServices.InitialProps> = (
    ...
    return (
        ...
        {access ? (
          ...
          <form className="lip-margin" id="vcModelSelectForm" onSubmit={(handleSubmit(handleModelSubmit))}>
            <Typography variant="h2" gutterBottom>
              Step 2: Choose a training model
            </Typography>
            <Typography variant="subtitle1" gutterBottom>
              You can choose a standard model or train your own custom model for better targetted results.
              If you are unsure about this, you can continue with the standard model.
            </Typography>
            <ToggleButtonGroup orientation="vertical" value={modelType} onChange={handleServiceChange} exclusive>
                <ToggleButton className="lip-button lip-model-button" value="standard">Standard Model</ToggleButton>
                <ToggleButton className="lip-button lip-model-button" value="custom">Custom Model</ToggleButton>
            </ToggleButtonGroup>
            {selectedCustom ?
            <div className="custom-div">
              <Typography variant="h3" gutterBottom>
                Choose existing model or 
                <a className="train-model" onClick={trainModel}> train a new model</a>
              </Typography>
              <Select
                native
                ...
                onChange={(event:any, child:any) => setModelId(event.target.value)}
                variant="outlined"
                defaultValue='none'
              > <option value="none" disabled>Choose custom model</option>
          ...
          <Dialog open={opneDialog} onClose={handleClose} aria-labelledby="form-dialog-title">
            <DialogTitle className="lip-title font-semibold text-xl leading-9" id="form-dialog-title">
              TRAIN A NEW MODEL
            </DialogTitle>
            <DialogContent>
              <form onSubmit={handleSubmit(newTrainingModel)}>
                <label className="lip-subtext font-semibold text-xl leading-9">Model name</label><br />
                <TextField 
                  ...
                  /><br />
                <label className="lip-subtext font-semibold text-xl leading-9">Upload file for custom training model</label>
                <Typography className="lip-subtext" variant="subtitle1" gutterBottom>
                  Supported Input Formats: .pdf, .txt, word file,  etc. File size limit - 20 mb
                </Typography>
                <div className="lip-button lip-dialog-button">
                  <Upload label="Upload Model" type="customModel" size={20} accept=".pdf,.txt,.doc" onUpload={setFilenameValue}/>
                </div>
                <div className="lip-button lip-dialog-button">
                  <GreenButton htmlType="submit" disabled={!props.dataFileId}>TRAIN MODEL</GreenButton>
                </div>
              </form>
            </DialogContent>
          </Dialog>
