...
import {
  RESOLVE_ERRORS_EMPLOYEES,
  RESOLVE_ERRORS_STUDENTS,
...
const ColumnMappingComponent: React.FC = (props: any) => {
    ...
    UniversityPortal.studentDataVerification(fileData).then((results: any) => {
      if (results.data.validationResult.length > 0) {
        ...
        if (userType === UserType.BUSINESS)
          router.push(RESOLVE_ERRORS_EMPLOYEES);
        else router.push(RESOLVE_ERRORS_STUDENTS);
      } else {
        if (userType === UserType.BUSINESS)
          router.push(CONFIRM_IMPORT_EMPLOYEES);
        else router.push(CONFIRM_IMPORT_STUDENTS);
      }
    });
  };
